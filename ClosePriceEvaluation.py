...
def myTradingSystem(DATE, OPEN, HIGH, LOW, CLOSE, VOL, exposure, equity, settings):
    ...
    sessions = 8
    sessionsDiff = numpy.diff(CLOSE[-sessions:,:],axis=0)
    longEquity = numpy.all(sessionsDiff < 0, axis=0)
    shortEquity = numpy.all(sessionsDiff > 0, axis=0)
